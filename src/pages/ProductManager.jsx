...
import { Search, Plus, X, Save, Trash2, Edit3, Grid, Smartphone, Maximize, Minimize2, List, ChevronDown, Package, Layers } from 'lucide-react';

const API_URL = import.meta.env.VITE_API_URL || 'https://vennial-backend.vercel.app/api/products';
...
const ProductManager = ({ category }) => {
  ...
  const initialFormState = {
    ...
    specs: {
      ...
      baseModel: '', modelNo: '', brandName: '', temperType: 'Normal', stripType: 'Single'
    }
  };
  ...
  useEffect(() => {
    ...
    setIsModalOpen(false); // Close modal if switching tabs
    setEditingProduct(null);

    axios.get(API_URL, { params: { category } })
      .then(res => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingProduct) {
        await axios.put(`${API_URL}/${editingProduct.id}`, formData, config);
  ...
  const filteredAndSortedProducts = (() => {
    // 0. Primary Category Filter (Double-check frontend logic)
    ...
    <div className="h-full flex flex-col bg-[#f3f4f6] p-4 md:p-6 page-transition">
        ...
        <div className="flex gap-2 shadow-sm w-full md:w-auto mt-4 md:mt-0 items-center">
          ...
          <div className="relative group">
            <select
              ...
              className="appearance-none bg-white border border-gray-300 text-gray-700 py-2.5 pl-3 pr-8 text-xs font-bold uppercase outline-none focus:border-[#00b894] sharp-edges cursor-pointer hover:border-[#00b894] transition-colors"
            >
          ...
          <button
            ...
            className="bg-[#00b894] text-white px-5 py-2.5 font-bold uppercase text-xs tracking-wider hover:bg-black transition-colors flex items-center gap-2 sharp-edges"
          >
            <Plus size={16} /> Add
          </button>
        </div>
      ...
      {loading ? (
        <div className="flex-1 flex flex-col justify-center items-center opacity-70">
          <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-12 h-12 border-4 border-gray-300 border-t-[#00b894] mb-4 sharp-edges" />
          <p className="font-bold text-[#00b894] animate-pulse">LOADING...</p>
        </div>
      ) : (
          ...
          <AnimatePresence>
            {filteredAndSortedProducts.map((item, index) => (
              <motion.div
                ...
                className="card-box bg-white border border-gray-200 shadow-sm hover:shadow-xl hover:border-[#00b894] transition-all duration-300 group flex flex-col sharp-edges"
              >
                <div className="p-3 border-b border-gray-100 bg-gray-50 group-hover:bg-[#00b894] group-hover:text-white transition-colors duration-300">
                  <div className="flex justify-between items-start">
                    ...
                    <Smartphone size={16} className="opacity-50 shrink-0 ml-1" />
                  </div>
                </div>
                <div className="p-3 flex-1">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 mb-1 text-[#00b894]">
                      <Layers size={12} />
                ...
                <div className="flex border-t border-gray-200">
                  ...
                  <button onClick={() => handleDelete(item.id)} className="w-10 py-2.5 text-gray-400 hover:bg-red-600 hover:text-white transition-colors flex items-center justify-center sharp-edges">
                    <Trash2 size={12} />
                  </button>
                </div>
              </motion.div>
            ...
            >
              ...
              <div className="bg-black text-white p-5 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                    <div className="bg-[#00b894] p-1 text-black">
                      <Package size={20} />
                    </div>
                ...
                <button onClick={() => setIsModalOpen(false)} className="hover:text-[#00b894] transition"><X size={24} /></button>
              </div>
              ...
              */}
                ...
                {isExpandedView ? (
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <div className="bg-white p-3 border-b border-gray-200 flex justify-between items-center shrink-0 shadow-sm">
                      ...
                      <button onClick={() => setIsExpandedView(false)} className="text-[#00b894] font-bold text-xs uppercase hover:underline flex items-center">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-gray-50 min-h-0">
                      <div className="flex flex-wrap gap-2 content-start">
                        {formData.compatibleDevices && formData.compatibleDevices.split(',').filter(t => t.trim() !== '').map((tag, index) => (
                          <span key={index} className="inline-flex items-center bg-white border border-gray-300 shadow-sm text-black text-sm font-bold px-3 py-2 tracking-wide sharp-edges group hover:border-[#00b894] transition-colors">
                    ...
                    <div className="p-4 bg-white border-t border-gray-200 shrink-0">
                      <input
                        ...
                        autoFocus
                        className="w-full p-4 border-2 border-gray-300 focus:border-[#00b894] outline-none font-bold text-lg sharp-edges"
                  ...
                  <div className="p-8">
                    <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {(category === 'Temper' || category === 'Screen Guard') && (
                        <>
                          <div className="md:col-span-2 p-4 bg-gray-50 border border-gray-200">
                            <label className="block text-xs font-black text-[#00b894] uppercase mb-2">Original Drawing (Master Model)</label>
                          ...
                          <div>
                            <label className="block text-xs font-black text-gray-400 uppercase mb-1">Glass Type / Filter</label>
                            <select name="temperType" value={formData.specs.temperType || 'Normal'} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none font-bold sharp-edges bg-white">
                              <option value="Normal">Normal Glass</option>
                              <option value="Curve">Curve Glass</option>
                            </select>
                          </div>
                          ...
                          <div className="md:col-span-2 grid grid-cols-2 gap-4 pt-4 border-t border-gray-100 mt-2">
                            ...
                            {['radiusTopLeft', 'radiusTopRight', 'radiusBottomLeft', 'radiusBottomRight'].map(field => (
                              <div key={field}><label className="text-[10px] font-bold text-gray-400 uppercase">{formatKey(field)}</label><input type="number" name={field} value={formData.specs[field] || ''} onChange={e => handleChange(e, true)} className="w-full p-2 border border-gray-300 focus:border-[#00b894] outline-none bg-gray-50 sharp-edges" /></div>
                            ))}
                          </div>
                        </>
                      )}

                      {(category === 'Combo Folder' || category === 'Combo/Display') && (
                        <>
                          <div>
                            <label className="block text-xs font-black text-gray-400 uppercase mb-1">Brand Name / Filter</label>
                            <select name="brandName" value={formData.specs.brandName || 'Redmi'} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none font-bold sharp-edges bg-white">
                              {['Redmi', 'Poco', 'Vivo', 'iQOO', 'Oppo', 'Realme', 'OnePlus', 'Techno', 'Infinix', 'Itel'].map(b => (
                                <option key={b} value={b}>{b}</option>
                              ))}
                            </select>
                          </div>
                          <div>
                            <label className="text-xs font-black text-[#00b894] uppercase">Model Number</label>
                            <input name="modelNo" value={formData.specs.modelNo || ''} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none sharp-edges font-bold" autoFocus />
                          </div>
                        </>
                      )}

                      {category === 'On Off Volume Strip' && (
                        <>
                          <div>
                            <label className="block text-xs font-black text-[#00b894] uppercase mb-1">Strip Type / Filter</label>
                            <select name="stripType" value={formData.specs.stripType || 'Single'} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none font-bold sharp-edges bg-white">
                              <option value="Single">Single Strip</option>
                              <option value="Separate">Separate Strip</option>
                            </select>
                          </div>
                          <div>
                            <label className="block text-xs font-black text-gray-400 uppercase mb-1">Base Model</label>
                            <input name="baseModel" value={formData.specs.baseModel || ''} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none sharp-edges font-bold" placeholder="e.g. Redmi Note 10" autoFocus />
                          </div>
                          <div>
                            <label className="text-xs font-black text-gray-400 uppercase">Model No</label>
                            <input name="modelNo" value={formData.specs.modelNo || ''} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none sharp-edges" />
                          </div>
                        </>
                      )}

                      {(category === 'Frame' || category === 'Center Panel' || category === 'CC Board' || category === 'Battery' || category === 'Back Case' || category === 'Phone Case' || category === 'PC Build' || category === 'Mobile Services') && (
                        <>
                          <div className="md:col-span-2">
                            <label className="block text-xs font-black text-[#00b894] uppercase mb-2">Build / Model Name</label>
                            <input name="baseModel" value={formData.specs.baseModel || ''} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none sharp-edges font-bold" placeholder="e.g. Redmi Note 11" autoFocus />
                          </div>
                          <div>
                            <label className="text-xs font-black text-gray-400 uppercase">Model No</label>
                            <input name="modelNo" value={formData.specs.modelNo || ''} onChange={e => handleChange(e, true)} className="w-full p-3 border border-gray-300 focus:border-black outline-none sharp-edges" />
                      ...
                      <div className="md:col-span-2 border-t-2 border-gray-100 pt-6 mt-2">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button
                            ...
                            className="text-[10px] font-bold text-[#00b894] uppercase flex items-center hover:bg-black hover:text-white px-3 py-1 border border-gray-200 transition-colors"
                          >
                          ...
                          <div className="flex flex-wrap gap-2 mb-2 max-h-32 overflow-y-auto custom-scrollbar">
                            {formData.compatibleDevices && formData.compatibleDevices.split(',').filter(t => t.trim() !== '').map((tag, index) => (
                              <span key={index} className="inline-flex items-center bg-black text-white text-xs font-bold px-2 py-1 tracking-wider sharp-edges">
                                {tag} <button type="button" onClick={() => removeTag(tag)} className="ml-2 hover:text-[#00b894]"><X size={12} /></button>
                              </span>
                            ))}
                          </div>
              ...
              <div className="p-5 bg-gray-100 border-t border-gray-200 flex justify-end gap-3 shrink-0 relative z-30">
                ...
                <button 
                  onClick={handleSave} 
                  disabled={submitting}
                  className="bg-[#00b894] text-white px-8 py-3 font-bold uppercase text-sm hover:bg-black transition flex items-center shadow-lg sharp-edges disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Save size={16} className="mr-2" /> {submitting ? 'Saving...' : 'Save Record'}
                </button>
              </div>
